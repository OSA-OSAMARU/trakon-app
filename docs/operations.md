...
### 2.7b 運営管理画面のアクセス許可（#204）

`/admin` は TRAKON 全体の利用状況（会員数・プラン別の内訳・プロジェクト数）を見るための
運営専用画面。**金額は扱わない**（売上・請求・返金は Stripe ダッシュボードが正）。

開けるアカウントは環境変数で指定する。

```
TRAKON_OPERATOR_EMAILS=ops@example.com,another@example.com
```

- **DB の列ではなく環境変数**で持つ。運営権限はデプロイ側で管理したいもので、
  アプリのデータ操作から昇格できる経路を作りたくないため
- 突き合わせるのは **Supabase JWT の検証済みメールアドレス**。`users` テーブルの値ではない
- **未設定なら誰も開けない。** 許可リスト外には `GET /api/v1/admin/metrics` が 404 を返し、
  画面の存在自体を気取らせない
- メールアドレスを変えたら、この値も更新する（JWT の値と突き合わせるため）

Vercel の Production / Preview 両方に登録する。Preview に入れておくと本番前に確認できる。
