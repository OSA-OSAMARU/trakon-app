...
| `/projects/:projectId/items/:itemId?view=calendar` **(#66)** | **月表示カレンダー（閲覧専用）**。同じ画面のタブ切り替え | ✅ | `CalendarMonthView`（`features/plans/calendar/`） |
...
> **v1.2（#148）**：Figma node 84:2 に合わせ、カードグリッドから**テーブル**へ改訂した。行は「遅延アイコン / プロジェクト名 + クライアント名 / 期間 / 進行責任者 / `>`」で、**行全体がスケジュールへのリンク**。アーカイブ・編集は行の「⋯」メニューへ寄せる。進行中／アーカイブ済みの切替は URL（`?tab=archived`）に載せる。
...
> **v1.2（#148）**：Figma node 73:57 に合わせて改訂。基本情報（**クライアント名**を追加）／制作物（連番付きの行）／参加者（氏名・所属・**通知先メール**・**職種**・**区分**）の 3 セクションと、**画面下に固定した作成内容のサマリ + 主操作**で構成する。**進行責任者**を入力済み参加者から必ず 1 名選ぶ（`progressManagerIndex` で送信）。
>
> node 78:18 の実装ノートに従い、① 追加ボタンは末尾に空行を 1 行足して先頭欄へフォーカス ② 完全な空行は作成時に無視 ③ 一部だけ入力された行は氏名欄にエラー ④ 最後の 1 行は削除不可 ⑤ 通知先メールは対応が必要なときだけ送り、作成時には送らない。
...
> **v1.2（#149）**：ヘッダーのタイトル右に**カラーテーマのピッカー**（node 56:2）を置く。色は状態ではなくユーザーの視覚整理の道具なので、変更は履歴に残さず即時反映する。
>
...
**重要な UX 制約**：
- **#255 改訂**：共有 URL は**一覧からいつでも確認・コピーできる**（暗号化して保管した生トークンから復元する、章5 §5.5.5）。旧仕様の「発行直後の1回のみ表示」は撤回した
- ただし**失効・期限切れのリンクには URL を出さない**（もう使えない文字列を誤ってコピーされるだけ）
- URL を復元できない行（#255 以前に発行された行／復号鍵が無い環境）は「再表示できません。必要なら発行し直してください」と案内する
...
### 4.4.13. 非会員URL 閲覧画面（`/share/:token`、v1.1 非会員URL前倒し／#256・#257 改訂）

**目的**：非会員URL閲覧者（クライアント）が、共有スコープに応じてスケジュールを**閲覧する**（FR-SHARE-05、UC-23、SR-AUTHZ-02）。

> **#257：閲覧専用**。#131 で追加した確認依頼／承認／差し戻しは**全プランで撤回**した（匿名のまま状態を変えられて「誰が承認したか」が記録に残らないため。03-api §3.6.10 / 05-security §5.4.5）。承認・差し戻しをしてほしい相手は閲覧者として組織へ招待する（Free でも 5 名まで）。

**権限**：未認証可。トークン自体が認可（`requireShareToken`、章5 §5.x）。

**処理フロー**：
1. URL アクセス → `GET /api/v1/share/:token` でスコープ・対象データ取得
2. トークン無効（404）→ 「このリンクは無効になりました。発行者にお問い合わせください」を中央に表示
3. 有効 → scope 範囲のスケジュールを**閲覧専用カレンダー**で表示。ボールをクリックすると閲覧専用の詳細パネルが開く（データを変える操作は無い）

**レイアウト（#256、Figma node 495:306）**：

会員側の画面はサイドバーが左の余白を作っているが、共有画面にはサイドバーが無く、カレンダーが画面の端に貼り付いていた。**淡色の地（`bg-surface-muted`）の上に角丸のカードを浮かせ、周囲に余白を取る**構成にする。

| 要素 | 内容 |
|---|---|
| 外側 | `bg-surface-muted` ＋ 余白（1440px 幅で左右 48px。狭い画面では詰める＝列が潰れるため） |
| カード | `rounded-2xl` ＋ `border` ＋ `shadow-card`。高さは画面いっぱいで、内側だけがスクロールする |
| カード上部 | 左にプロジェクト名（`text-heading-page`）と期間、右に「共有用」バッジと有効期限。狭い画面では縦に積む（名前を省略すると何の共有か分からなくなる） |
| カード内 | スケジュールボード。カードの内側にも余白を取る（Figma node 496:317 は左 32px） |

**コンポーネント**：
- `SharePage`（ルート、`/share/:token`）— 地とカード（`ShareCanvas`）、見出し、失効表示を持つ
- `ShareSchedule` — 認証済み画面と同じ `ScheduleBoard` を閲覧専用モードで使う（#143 で共通化済み）
- `SharePlanDetailSheet` — 予定の閲覧専用パネル（#257 で `ShareActionModal` を置き換え）

**API**：`GET /api/v1/share/:token` のみ（#257）。

**個人情報**：予定 DTO に載るのは氏名・所属名まで。**メールアドレス・職種は共有画面へ流さない**（#159）。参加者一覧 API を呼ばないため、ホバーカードも出ない。

**SEO 対策**：`<meta name="robots" content="noindex, nofollow, noarchive">` を `/share/:token` のマウント時に注入する（章5 §5.x のクローラ防止と整合）。レスポンスヘッダにも `X-Robots-Tag` を付ける。
...
スケジュールカードとサイドモーダルの担当者（実施者／承認者／進行責任者）にマウスオーバーすると、
**名前／所属名／メールアドレス／職種** を出す（`features/projects/MemberProfileCard.tsx`）。
値は参加者一覧 API から取り、アカウント紐付け済みならマイページの内容が read-through で反映される（#156）。

| 論点 | 方針 |
|---|---|
| **非会員への漏洩** | **カードはメールアドレスを含むので、共有ページ（`/share/:token`）では出さない。** `ScheduleBoard` / `BallChip` は共有ページと共用しているため、`memberById` を渡されたときだけホバーを付ける形にした。共有ページは参加者一覧を取得しないので**構造的に渡しようがない**（オプトインにして事故を防ぐ） |
| **予定 DTO に足さない** | `services/plans.ts` の `MemberRef` は `shareAccess.ts` がそのまま再利用しているため、ここにメール等を足すと非会員へ筒抜けになる。ホバーの値は参加者一覧 API 側から取る。統合テストでレスポンス全体を走査して見張っている |
| **キーボード到達性** | サイドモーダル側はタブ順に含める（3 件だけ）。スケジュールカード側は 1 予定あたり最大 3 人分あり、全部を入れるとボードの移動が困難になるため含めない。カード自体は Enter で詳細を開けるので、そちら経由で到達できる |

---

## 4.6b. 月表示カレンダー（#66）

縦型スケジュールと**並立する第 2 のビュー**。既定は縦型のままで、`?view=calendar` で切り替える。

### なぜ別ビューとして持つのか

| | 縦型スケジュール | 月表示カレンダー |
|---|---|---|
| 軸 | 行＝日付 × 列＝制作物 | 月グリッド（7 列 × 週） |
| 答える問い | 「工程がどう連なり、いま誰の番か」 | 「今月なにがあるか」 |
| 操作 | ドラッグで日程変更・後続の紐付け | **閲覧専用** |

用途が重ならないので両方を持つ意味がある。ガントチャート（行＝予定 × 列＝日付）は
縦型と情報が大きく重なるため、**まず月表示だけを出して必要性を見てから**判断する。

### 閲覧専用にした理由

縦型が持つドラッグ操作をここへ持ち込むと、**同じ操作の実装が 2 系統**になり、
片方だけ直る事故が起きる。カードを押せば既存のボール詳細ドロワーが開くので、
そこから先の操作は 1 箇所に集約される。

### 停滞の可視化

ただの月間カレンダーにしないための要。`scheduleLayout.ts` の `isOverdue()` を共有し、
**3 段階で見せる**。

1. 月の合計（ヘッダー右「期限超過 N 件」）— グリッドを走査せずに「今月は滞っている」が分かる
2. 日ごとの件数（セル右上の赤い数字）— どの日に固まっているか
3. 予定ごとの強調（チップの太い赤枠）— どれが滞っているか

### レイアウト計算

`calendar/calendarLayout.ts` が持つ。縦型の `scheduleLayout.ts` は「縦が日付」前提の
計算で流用できないため、**月グリッド専用**に分けている。

- 週の始まりは日曜。**必要な週数だけ**返す（5 週の月は 5 行）。空の行は俯瞰の邪魔になる
- 期間のある予定は**跨る日すべて**に出す
- 1 日あたりのチップは 3 件まで。超えた分は「他 N 件」に畳んで**行の高さを揃える**
...
**実装（#149 で完了）**：`plans.color_theme`（10 値の CHECK、NULL 許容）にユーザーの選択を保存する。NULL のときは `plans.category`（6 値）から既定テーマを導出する。解決は `features/plans/planTheme.ts` の `resolvePlanTheme()` に集約する。

選択 UI は `components/trakon/ScheduleThemePicker`。ボール詳細ドロワーのヘッダー（Figma node 56:2）と、予定の作成・編集フォームに置く。「カテゴリの既定色に戻す」で NULL へ戻せる。
...
| 2026-09-26 | **#255 / #256 / #257 反映** | §4.4.12 SC-16 の「URL は発行直後の1回のみ表示」を撤回し、一覧からいつでもコピーできる仕様へ改訂（#255。失効・期限切れには出さない／復元できない行は案内する）／§4.4.13 非会員URL 閲覧画面を全面改訂。Figma node 495:306 に合わせて**淡色の地 + 角丸カード + 周囲の余白**の構成にし（#256。サイドバーが無いためカレンダーが画面の端に貼り付いていた）、見出しを「プロジェクト名・期間・共有用バッジ・有効期限」の 2 カラムへ。共有リンクを**閲覧専用**として再定義し `ShareActionModal` を `SharePlanDetailSheet` へ置き換えた（#257）。 |
