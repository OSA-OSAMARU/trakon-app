...
PRD §8.2 の `attachments`。業務上のファイル（支給素材・確認用の書き出し）を扱うため、
プロフィール画像より緩い一方で、**実行されうる形で配信しない**ことを最優先にしている。

| 項目 | 方針 |
|---|---|
| 保存先 | Supabase Storage の**非公開バケット** `attachments`。キーは `{projectId}/{planId}/{uuidv7}.{ext}` |
| 公開範囲 | **公開 URL を持たせない**。ダウンロードは都度発行する有効期限 **10 分**の署名付き URL（直リンク禁止） |
| 配信 | 署名付き URL に必ず `download` を指定し、**`Content-Disposition: attachment` を強制**する。SVG / HTML を添付されてもブラウザ内で描画・実行されない（保存型 XSS の遮断はここが要） |
| 形式の検証 | **許可リストではなく拒否リスト**。制作現場のファイル（psd / ai / indd / sketch / mov …）は種類が読めず増え続けるため、許可リストにすると日常的に詰まる。拒否するのは「クリックで走る」形式（exe / sh / vbs / js / dmg …）のみ |
| サイズ・件数 | 1 ファイル **20MB** / 1 予定あたり **20 件** |
| アクセス制御 | Storage の RLS ポリシーは書かない。認可はミドルウェアチェーン（`attachment.create` / `attachment.delete`）に一本化する |
| 削除の権限 | **自分が添付したものだけ**。管理者は他人のものも削除できる（ボール操作と同じ 2 段判定） |
| 削除時 | 台帳は論理削除、**Storage 上の実体は削除する**。実体を残すと「消したはずのファイル」が署名付き URL で読めてしまう |
| 非会員共有ページ | `/share/:token` には**出さない**（会員の業務ファイルであり、共有ページは閲覧に限る） |

> **マジックバイト検証を行わない理由**：プロフィール画像は「PNG/JPEG/WebP のいずれか」と
> 言い切れるので実体を確認できる。添付は形式が閉じておらず、判定できないものを弾くと
> 正規のファイルが通らなくなる。代わりに**配信側で実行させない**方向で守っている。
