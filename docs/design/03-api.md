...
氏名・メール・所属・職種は**受け取らない**。アカウント側（`users`）が正であり（#156、氏名は #254）、
...
#### `POST /api/v1/projects/:projectId/items/:itemId/copy`

制作物を**予定ごと複製**する（#200）。スケジュールの「カラム」をまるごと写し取る操作。
...
**処理**：
1. 制作物を `「〜 のコピー」` という名前で作る。期間（`start_date` / `end_date`）と `item_type` は引き継ぎ、`sort_order` は末尾
2. 配下の**削除されていない予定をすべて**複製する。日付・カテゴリ・色・担当 3 役割・メモを引き継ぐ
3. 予定どうしの**後続の紐付けを新しい ID へ張り替える**

**引き継がないもの**：
- `ball_events`（ボールの履歴）— 「実際に誰が何をしたか」の記録なので、複製で架空の履歴を作らない
- TOSS の FROM/TO スナップショット、`status` / `completed_at` — 複製された予定はすべて未着手から始まる
- **複製範囲の外を指していた後続の紐付け** — 写すとコピーが元のラインへ合流してしまう

**レスポンス**：201、作成された制作物の DTO。

**エラー**：404（他プロジェクトの制作物・存在しない）。
