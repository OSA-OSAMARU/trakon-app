...
| #145 | 2026-08-23 | **Figma デザインシステム反映（第5段：ボール詳細ドロワー）**。SC-08 を概要 / 履歴の 2 タブ構成へ。主要操作をフッターの WorkflowButton に、取り消し系・削除をヘッダーの「⋯」メニューへ整理。状態機械と役割別の操作可否は #131 から不変。 | — |
| #146 | 2026-08-23 | **Figma デザインシステム反映（第6段：ダッシュボード）**。SC-09 を「次に必要な行動」で並べる 4 列ボードへ改訂。列の導出を `packages/shared` に共有関数として追加し、`tossed` は後続予定と二重計上になるためボードから除外。「要対応のみ」トグルと進行責任者表示のため dashboard DTO に 2 項目を追加。 | — |
...
| #254 | 2026-09-26 | **表示名を全画面で統一（issue #254）**。氏名（画面に出る名前）の正を `users.display_name` に一本化し、`project_members.name` は**アカウント未紐付けの参加者と、表示名が空のときのフォールバック**に降格。`resolveMemberProfile()` が `name` も解決するようにし、予定・ダッシュボード・添付・参加者一覧・メンバー管理のすべてを同じ関数に通す。`users` の read-through 用 select を `server/lib/memberProfile.ts` に集約し、3 箇所に重複していた `PLAN_INCLUDE` を `services/plans.ts` の 1 つへ統合（include 漏れによる再発防止）。02-database §2.4.1 を改訂。 | — |
| #255 | 2026-09-26 | **発行済み共有リンクの URL を一覧で再表示できるようにする（issue #255）**。`share_links.token_cipher` を追加し、生トークンを AES-256-GCM（鍵はアプリ env `SHARE_TOKEN_ENCRYPTION_KEY`、DB には置かない）で保管する。トークンの**照合は従来どおり `token_hash` の完全一致のみ**で、暗号文は表示専用。「生トークンは DB に保存しない」という 05-security の方針を**共有リンクに限って改訂**（招待トークンは対象外）。一覧 API に `url` を追加し、SC-16 で URL のコピーを可能にした。02-database §2.4.X・03-api §3.6.9・05-security §5.4.5 を改訂し §5.5.5 を新設。 | — |
| #257 | 2026-09-26 | **プランごとの仕様変更（issue #257）**。① **共有リンク（非会員）は全プランで閲覧専用に戻した**。#131 で許可した確認依頼 / 承認 / 差し戻しを撤回し（匿名のまま状態を変えられて「誰が承認したか」が記録に残らないため）、`POST /share/:token/plans/:planId/*` を削除。承認してほしい相手は閲覧者として招待する運用へ一本化。② **Free の `viewerLimit` を 0 → 5**。Free でも閲覧者を 5 名まで招待できるようにした。座席 1 はオーナー本人で埋まるため、管理者・編集者として招待できない制約は座席数から自然に導かれる。③ `Entitlement.invitableProjectRoles` を追加し、画面の権限選択肢を判定関数 1 か所から供給（§7.6.4）。④ **`project_members.role_type` は座席の種類を超えられない**ようにした（`SEAT_ROLE_REQUIRED` / 409）。「閲覧者として招待 → プロジェクトで編集者に昇格」で座席上限を素通りできる穴を塞いだ。Personal / Team の振る舞いは従来どおり。03-api §3.6.10・05-security §5.4.5 / §5.6.1・07-billing §7.2 / §7.3.2 を改訂。 | — |
| #256 | 2026-09-26 | **共有URLで表示するカレンダーのデザイン修正（issue #256）**。Figma node 495:306 に合わせ、共有画面を**淡色の地の上に角丸カードを浮かせて周囲に余白を取る**構成へ改訂。会員側はサイドバーが左の余白を作っているが共有画面にはサイドバーが無く、カレンダーが画面の端に貼り付いていた。見出しを「プロジェクト名・期間」＋「共有用バッジ・有効期限」の 2 カラムにし、狭い画面では縦に積む（プロジェクト名を省略すると何の共有か分からなくなるため）。04-frontend §4.4.13 を全面改訂。 | — |
| #258 | 2026-09-26 | **組織招待時に参加するプロジェクト名を表示する（issue #258）**。`GET /invitations/:token` に `projects` を追加し、受諾画面と招待メールの双方に「参加するプロジェクト」を出す。組織単位の招待では複数になる（0 件＝組織に入るだけ、もありうる）。求め方は**受諾処理が実際に紐づける行と同じ条件**（`findInvitedProjects()` に集約）にして、案内と結果がずれないようにした。メールは `projectName`（実体は組織名だった）を `organizationName` + `projectNames` に置き換え、件名は「1 件目 ほか N 件」、本文は全件。03-api §3.6.x を改訂。 | — |
