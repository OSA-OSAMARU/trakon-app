...
| 14 | カテゴリ・後続紐付け運用（v1.1 / **#131**） | **インフラ層は新規対応なし**（DB スキーマと API のみ、章2・章3 で対応済み） | 既存インデックス追加（章2 §2.8）で対応。~~後続自動 TOSS 件数の指標~~ は #117 の自動連鎖廃止に伴い不要（#131 では承認→TOSS の状態機械件数を Phase 1 で指標化検討） |
