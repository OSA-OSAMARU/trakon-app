...
| FR-PRJ-04 | 開始日 / 終了日の変更で範囲外になる予定が存在する場合、**変更を拒否する**（#155 で「警告を表示」から変更。期間外の予定は縦型スケジュールに描画先の行が無く、必ず表示が壊れるため） | 1 | プロジェクト仕様書 v1.2 §4, §10 ／ #155 |
...
- **URL**：`/settings/profile`（マイページ）／`/settings/billing`（プラン・お支払い、SC-18）／`/settings/members`（メンバー管理、#160）。#156 でタブ形式をやめ、サイドバーのアカウントメニューから各ページへ直接遷移する
- **画面項目**：プロフィール／パスワード変更／MFA（Phase 2）／**組織名・組織メンバー一覧と組織ロール（v1.4）**／**プランと請求（SC-18 へ、v1.4）**／**非会員URL共有 機能 On/OFF（Phase 2、組織管理者のみ）**

### SC-18: プラン・お支払い（✅ Phase 0.5、v1.4 新規／#156 で画面名を Figma に合わせて改称）
